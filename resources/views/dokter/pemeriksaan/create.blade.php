...
@section('header-title', 'Pemeriksaan Pasien')
...
@section('content')
    <div class="row">
            ...
            <div class="card-custom mb-4">
                ...
                <div class="card-body p-4">
                    <div class="mb-3">
                        ...
                        <div class="fs-5 fw-bold">{{ $pendaftaran->pasien->nama_lengkap ?? $pendaftaran->pasien->nama }}</div>
                    </div>
                    <div class="mb-3">
                        <label class="text-muted small text-uppercase fw-bold">No. Rekam Medis</label>
                        <div class="fs-6">{{ $pendaftaran->pasien->no_rekam_medis ?? $pendaftaran->pasien->no_rm }}</div>
                    </div>
                    <div class="mb-3">
                        <label class="text-muted small text-uppercase fw-bold">Usia</label>
                        <div class="fs-6">{{ \Carbon\Carbon::parse($pendaftaran->pasien->tanggal_lahir)->age }} Tahun</div>
                    </div>
                    <div class="mb-0">
                        <label class="text-muted small text-uppercase fw-bold">Keluhan Utama</label>
                        <div class="p-2 bg-light rounded border">{{ $pendaftaran->keluhan }}</div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-0">
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                            ...
                            <span class="fw-bold">{{ $pendaftaran->vitalSign->tekanan_darah }} mmHg</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                            <span>Suhu Tubuh</span>
                            <span class="fw-bold">{{ $pendaftaran->vitalSign->suhu }} °C</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                            <span>Denyut Nadi</span>
                            <span class="fw-bold">{{ $pendaftaran->vitalSign->nadi }} x/menit</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                            <span>Pernapasan</span>
                            <span class="fw-bold">{{ $pendaftaran->vitalSign->pernapasan }} x/menit</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                            <span>Berat Badan</span>
                            <span class="fw-bold">{{ $pendaftaran->vitalSign->berat_badan }} kg</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                            <span>Tinggi Badan</span>
                            <span class="fw-bold">{{ $pendaftaran->vitalSign->tinggi_badan }} cm</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="col-md-8">
            ...
            <div class="card-custom">
                <div class="card-header-custom">
                    <i class="fas fa-stethoscope me-2"></i>Formulir Pemeriksaan
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('dokter.pemeriksaan.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="anamnesa" class="form-control" rows="3" required placeholder="Riwayat penyakit sekarang, dahulu, keluarga..."></textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Pemeriksaan Fisik (O)</label>
                            <textarea name="pemeriksaan_fisik" class="form-control" rows="3" required placeholder="Hasil pemeriksaan fisik head-to-toe..."></textarea>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="diagnosis_utama" class="form-control" required placeholder="Diagnosa ICD-10">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold">Diagnosa Sekunder (Opsional)</label>
                                <input type="text" name="diagnosis_sekunder" class="form-control" placeholder="Diagnosa tambahan">
                            </div>
                        </div>
                        
                        <div class="mb-3">
                            <label class="form-label fw-bold">Tindakan / Terapi (P)</label>
                            <textarea name="tindakan" class="form-control" rows="3" required placeholder="Tindakan medis yang dilakukan..."></textarea>
                        </div>
                        ...
                        <div class="row bg-light p-3 rounded mb-3 border">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="rencana_tindak_lanjut" class="form-select">
                                    ...
                                    <option value="Kontrol">Kontrol Kembali</option>
                                    <option value="Rujuk">Rujuk ke RS Lain</option>
                                    <option value="Pulang">Boleh Pulang</option>
